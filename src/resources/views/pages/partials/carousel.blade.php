<section class="slider">
    @if(count($mainPageImages)>0)
        @foreach($mainPageImages as $pathToImage)
            <div class="slide"
                 style="background-image: url('{{$pathToImage->path}}');">
                <div class="content">
                    @php
                    $category = App\Category::find($pathToImage->category_id);
                    @endphp
                    <h2>
                        @if($category != null)
                            {{$category->category}}
                        @endif
                    </h2>
                    {{--<p>Some small description...</p>--}}
                    {{--<a href="/products.html" class="btn btn-primary-outline"></a>--}}
                    <a href="{{ url('category', $pathToImage->category_id) }}"
                       class="btn btn-primary-outline">ПЕРЕЙТИ</a>
                </div>
            </div>
        @endforeach
    @endif
</section>


{{--<section class="tiles">--}}
    {{--<div class="heading">--}}
{{--        <h2>{{trans('messages.special')}}</h2>--}}
        {{--<hr/>--}}
    {{--</div>--}}
    {{--@foreach($mainPageImagesStatic as $img)--}}
        {{--<img src="{{ $img->path }}" class="img-responsive" style="width: 100%" alt="">--}}
    {{--@endforeach--}}
{{--</section>--}}


{{--<!-- Carousel -->--}}
{{--<div id="carousel1" class="carousel slide carousel-fade hoverable">--}}

{{--<!-- Indicators -->--}}
{{--<ol class="carousel-indicators">--}}
{{--<li data-target="#carousel1" data-slide-to="0" class="active"></li>--}}
{{--<li data-target="#carousel1" data-slide-to="1"></li>--}}
{{--<li data-target="#carousel1" data-slide-to="2"></li>--}}
{{--<li data-target="#carousel1" data-slide-to="3"></li>--}}
{{--</ol>--}}

{{--<!-- Wrapper for slides -->--}}
{{--<div class="carousel-inner z-depth-2" role="listbox">--}}

{{--<!-- First slide -->--}}
{{--<div class="item active" id="tf-home">--}}
{{--<div class="view overlay hm-blue-slight">--}}
{{--<a><img src="/store/src/public/images/slider/main_2.jpg" alt="Shopify Welcome">--}}
{{--<div class="mask waves-effect waves-light"></div>--}}
{{--</a>--}}
{{--<div class="carousel-caption hidden-xs">--}}
{{--<div class="animated fadeInDown">--}}
{{--<h1><strong><span class="color">Store</span></strong></h1>--}}
{{--<p class="lead">Shop your favorite electronics, brands and more!</p>--}}
{{--</div>--}}
{{--</div>--}}
{{--</div>--}}
{{--</div>--}}


{{--<div class="item" id="tf-home">--}}
{{--<div class="view overlay hm-blue-slight">--}}
{{--<a><img src="/store/src/public/images/slider/iPhone_1.jpg" alt="iPhone 6s">--}}
{{--<div class="mask waves-effect waves-light"></div>--}}
{{--</a>--}}
{{--<div class="carousel-caption hidden-xs">--}}
{{--<div class="animated fadeInDown">--}}
{{--<h5>iPhone 6s</h5>--}}
{{--</div>--}}
{{--</div>--}}
{{--</div>--}}
{{--</div>--}}
{{--<!-- /.item -->--}}

{{--<!-- Second slide -->--}}
{{--<div class="item" id="tf-home">--}}
{{--<div class="view overlay hm-blue-slight">--}}
{{--<a><img src="/store/src/public/images/slider/Xbox_Controller_2.jpg" alt="Xbox One Elite Controller">--}}
{{--<div class="mask waves-effect waves-light"></div>--}}
{{--</a>--}}
{{--<div class="carousel-caption hidden-xs">--}}
{{--<div class="animated fadeInDown">--}}
{{--<h5>The new Xbox One Elite Controller</h5>--}}
{{--</div>--}}
{{--</div>--}}
{{--</div>--}}
{{--</div>--}}
{{--<!-- /.item -->--}}

{{--<!-- Third slide -->--}}
{{--<div class="item" id="tf-home">--}}
{{--<div class="view overlay hm-blue-slight">--}}
{{--<a><img src="/store/src/public/images/slider/Astro_3.jpg" alt="Astros">--}}
{{--<div class="mask waves-effect waves-light"></div>--}}
{{--</a>--}}
{{--<div class="carousel-caption hidden-xs">--}}
{{--<div class="animated fadeInDown">--}}
{{--<h5>Astros</h5>--}}
{{--</div>--}}
{{--</div>--}}
{{--</div>--}}
{{--</div>--}}
{{--<!-- /.item -->--}}
{{--</div>--}}
{{--<!-- /.carousel-inner -->--}}

{{--<!-- Controls -->--}}
{{--<a class="left carousel-control new-control" href="#carousel1" role="button" data-slide="prev">--}}
{{--<span class="fa fa fa-angle-left waves-effect waves-light"></span>--}}
{{--<span class="sr-only">Previous</span>--}}
{{--</a>--}}
{{--<a class="right carousel-control new-control" href="#carousel1" role="button" data-slide="next">--}}
{{--<span class="fa fa fa-angle-right waves-effect waves-light"></span>--}}
{{--<span class="sr-only">Previous</span>--}}
{{--</a>--}}

{{--</div>--}}
{{--<!-- /.carousel -->--}}


